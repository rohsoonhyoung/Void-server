...
import (
	...
	"github.com/bujor2711/Void-server/dungeon"
...
func (h *OpenHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	for _, action := range gjson.Get(script, "actions").Array() {
		ac := int(action.Int())
		if ac == 3330 && !database.TriviaEventCounting {
			continue
		}
...
func (h *PressButtonHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if script != "" {
		...
		if c.IsQuestMenuOpened {
			...
			fmt.Printf("QuestActID: %d", quest.ID)
			if quest != nil {
					...
					if mquest != nil {
						if mquest.QuestState == 3 {
							...
							if len(questMaterials) <= 0 && !mQuest.DropFromMobs {
								...
								//IT MUST BE HERE IF THE NPC GIVES THE QUEST ITEM
							}
							...
							mquest.Update()
							return resp, nil
						} else if mquest.QuestState == 4 { //Quest delivered
							resp, err = finishQuest(quest.ID, c)
							//c.AddPlayerQuests()
							if err != nil {
								log.Print("Leadási error")
							}
							err = c.AddPlayerQuests()
							if err != nil {
								log.Print(fmt.Sprintf("Error with load: %s", err))
							}
		...
		case 3329:
			if database.TriviaEventCounting {
				resp = database.GetNPCMenu(npcID, 1170980005, 0, []int{})
				c.AddToTrivia()
			} else {
				resp = database.GetNPCMenu(npcID, 1170980006, 0, []int{})
			}
