...
func (h *AdvancedFusionHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	fusion := database.Fusions[int(items[0].ItemID)]
	if success || (!success && fusion.DestroyOnFail) {
		for _, id := range slotIDs {
			itemData := s.Character.DecrementItem(id, 1)
	...
	if special != nil {
		itemData := s.Character.DecrementItem(specialSlot, 1)
		if err != nil {
			return nil, err
		}
...
func (h *DismantleHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	resp := utils.Packet{}
	dismantleData, success, err := s.Character.Dismantle(slots[itemSlot], special, false)
	if err != nil {
		return nil, err
	}
	if !success {
