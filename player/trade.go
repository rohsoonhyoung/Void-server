...
func (h *SendTradeRequestHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if s.Character.Map == 251 || s.Character.Map == 243 {
	...
	receiver := database.FindCharacter(user.ConnectedServer, pseudoID)
	if receiver == nil {
		return database.TRADE_CANCELLED, nil
...
func (h *RespondTradeRequestHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	sender := database.FindCharacter(user.ConnectedServer, pseudoID)
...
func (h *AddTradeItemHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	if item.Activated || item.InUse || info.Tradable != 1 {
		return nil, nil
	}
...
func (h *AcceptTradeHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	f, err := os.OpenFile("logs/trade_log.txt", os.O_APPEND|os.O_CREATE|os.O_WRONLY, 0644)
	if err != nil {
		log.Println(err)
	}
	defer f.Close()
	tradelogger := log.New(f, "", log.LstdFlags)
	...
	_, err = trade.Receiver.Character.FindFreeSlots(len(trade.Sender.Slots))
	if err != nil {
		return messaging.SystemMessage(messaging.DISMINTLE_NOT_ENOUGH_SPACE), nil
	}
	_, err = trade.Sender.Character.FindFreeSlots(len(trade.Receiver.Slots))
	if err != nil {
		return messaging.SystemMessage(messaging.DISMINTLE_NOT_ENOUGH_SPACE), nil
	}
	...
	if trade.Sender.Accepted && trade.Receiver.Accepted && !trade.Completing {
		...
		if success {
			...
			for _, slotID := range trade.Sender.Slots {
				...
				if item.Activated || item.InUse || iteminfo.Tradable != 1 {
					break
				}
				...
				*senderSlots[slotID] = *database.NewSlot()
				...
				amount := strconv.Itoa(int(item.Quantity))
				itemid := strconv.Itoa(int(item.ItemID))
				text := fmt.Sprintf(trade.Sender.Character.Name + "(" + trade.Sender.Character.UserID + ")" + " traded item (" + itemid + ")(" + iteminfo.Name + ") QTY: " + amount + " TO : " + trade.Receiver.Character.Name + "(" + trade.Receiver.Character.UserID + ")")
				tradelogger.Println(text)
			}
			...
			amount := strconv.Itoa(int(rGold))
			if rGold > 0 {

				text := fmt.Sprintf(trade.Sender.Character.Name + "(" + trade.Sender.Character.UserID + ")" + " traded gold (" + amount + ") TO : " + trade.Receiver.Character.Name + "(" + trade.Receiver.Character.UserID + ")")
				tradelogger.Println(text)
			}
		}
		...
		if success {
			...
			for _, slotID := range trade.Receiver.Slots {
				...
				*receiverSlots[slotID] = *database.NewSlot()
				...
				amount := strconv.Itoa(int(item.Quantity))
				itemid := strconv.Itoa(int(item.ItemID))
				text := fmt.Sprintf(trade.Receiver.Character.Name + "(" + trade.Receiver.Character.UserID + ")" + " traded item (" + itemid + ") QTY: " + amount + " TO : " + trade.Sender.Character.Name + "(" + trade.Sender.Character.UserID + ")")
				tradelogger.Println(text)
			}
			...
			amount := strconv.Itoa(int(rGold))
			text := fmt.Sprintf(trade.Receiver.Character.Name + "(" + trade.Receiver.Character.UserID + ")" + " traded gold (" + amount + ") TO : " + trade.Sender.Character.Name + "(" + trade.Sender.Character.UserID + ")")
			tradelogger.Println(text)
		}
