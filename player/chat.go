...
import (
	...
	"github.com/bujor2711/Void-server/npc"
	"github.com/bujor2711/Void-server/server"
...
func (h *ChatHandler) cmdMessage(s *database.Socket, data []byte) ([]byte, error) {
	...
	if parts := strings.Split(h.message, " "); len(parts) > 0 {
		...
		case "startci":
			...
			events.StartCiEventCountdown(600)
		case "setinjury":
			if s.User.UserType < server.HGM_USER {
				return nil, nil
			}
		...
		case "trivia":
			...
			database.StartTriviaCounter(600)
			...
			for _, npcPos := range database.GetNPCPostions() {
				...
				for k := 1; k <= 4; k++ {
					for i := 0; i < int(npcPos.Count); i++ {
						...
						newai := &database.AI{ID: len(database.AIs), HP: npc.MaxHp, Map: npcPos.MapID, PosID: npcPos.ID, RunningSpeed: float64(npc.RunningSpeed), Server: k, WalkingSpeed: float64(npc.WalkingSpeed), Faction: 0, CanAttack: true}
						database.GenerateIDForAI(newai)
						newai.OnSightPlayers = make(map[int]interface{})
