...
import (
	...
	"github.com/bujor2711/Void-server/gold"
...
type (
	...
	DonateGoldToGuildHandler   struct{}
)
...
func (h *DonateGoldToGuildHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	gold := uint64(utils.BytesToInt(data[10:18], true))
	s.Character.ClanGoldDonation += gold
	if !s.Character.SubtractGold(uint64(gold)) {
		return nil, nil
	}
	guild.GoldDonation += gold
	guild.Update()
...
func (h *GuildRequestHandler) Handle(s *database.Socket, data []byte) ([]byte, error) {
	...
	character := database.FindCharacter(user.ConnectedServer, pseudoID)

	guild, err := database.FindGuildByID(s.Character.GuildID)
	if err != nil {
		return nil, err
	}
	// if guild.MemberCount >= database.MAX_GUILD_MEMBERS { // max members reached
	// 	return messaging.SystemMessage(messaging.MAX_GUILD_MEMBERS_REACHED), nil
	// }
