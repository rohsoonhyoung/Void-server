...
func GetAllItems() error {
	...
	for index, row := range rows {
		...
		item := &Item{
			...
			MinUpgradeLevel:          uint64(utils.StringToInt(row[29])),
			SpecialItem:              int64(utils.StringToInt(row[37])),
			...
			PVPdefRate:               utils.StringToInt(row[141]),
			PVPsdefRate:              utils.StringToInt(row[142]),
			Buff:                     utils.StringToInt(row[144]),
			BeastType:                utils.StringToInt(row[145]),
