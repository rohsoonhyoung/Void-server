		...
		for _, id := range ids {
			...
			if funk.Contains(WarStonesIDs, npc.ID) {
				...
				if distance < 20 {
					...
					if c.Faction == 1 {
						if !funk.Contains(WarStones[int(ai.PseudoID)].NearbyZuhangs, c) {
							WarStones[int(ai.PseudoID)].NearbyZuhangs = append(WarStones[int(ai.PseudoID)].NearbyZuhangs, c)
						}
					} else {
						if !funk.Contains(WarStones[int(ai.PseudoID)].NearbyShaos, c) {
							WarStones[int(ai.PseudoID)].NearbyShaos = append(WarStones[int(ai.PseudoID)].NearbyShaos, c)
						}
					}
					if c.Socket.Stats.HP <= 0 {
						if c.Faction == 1 {
							if funk.Contains(WarStones[int(ai.PseudoID)].NearbyZuhangs, c) {
								WarStones[int(ai.PseudoID)].RemoveZuhang(c)
							}
						} else {
							if funk.Contains(WarStones[int(ai.PseudoID)].NearbyShaos, c) {
								WarStones[int(ai.PseudoID)].RemoveShao(c)
							}
						}
					}
				} else {
