...
type Character struct {
	...
	TriviaAnswer   int  `db:"-"` //	0:false	1:true	3:not answered
	CanRun         bool `db:"-"`
...
func FindCharacterByName(name string) (*Character, error) {
	...
	character := &Character{}
	err := pgsql_DbMap.SelectOne(&character, "select * from hops.account where login = $1", name)
	if err != nil {
		return nil, err
	}
...
func (c *Character) SwapItems(where, to int16) ([]byte, error) {
	...
	if whereItem.SlotID == where && (where == 317 || where == 318 || where == 319) && (toItem.SlotID != 317 || toItem.SlotID != 318 || toItem.SlotID != 319) {
		return nil, nil
	}
	...
	for _, i := range indexes {
		...
		if item.ItemID != 0 {
			...
			for _, id := range ids {
				...
				/*if item.HtType == 21 { //pet item
					st.PetExpMultiplier += item.PvpDampReduction / 100
					pet := slots[i].Pet
					if pet == nil {
						continue
					}
					pet.ArtsDEF += pet.ArtsDEF * item.ArtsDefRate
					pet.DEF += pet.DEF * item.DefRate
...
func (c *Character) ChangeMap(mapID int16, coordinate *utils.Location, args ...interface{}) ([]byte, error) {
	...
	if Maps[int(mapID)].HousingMap == 2 { // player house

	} else if funk.Contains(sharedMaps, mapID) { // shared map
	...
	if Maps[int(c.Map)] != nil && Maps[int(c.Map)].IsDark {
...
func (c *Character) BSProduction(book *InventorySlot, materials []*InventorySlot, special *InventorySlot, prodSlot int16, bookSlot, specialSlot int16, materialSlots []int16, materialCounts []uint) ([]byte, error) {
	...
	if !ok {
		return nil, errors.New("Invalid production id")
	}
...
func (c *Character) UseConsumable(item *InventorySlot, slotID int16) ([]byte, error) {
	...
	case WRAPPER_BOX_TYPE:
		...
		} else if item.ItemID == 17502588 { //trivia true
			if TriviaEventStarted {
				go c.AnswerTriviaQuestion(1)
			}
			goto FALLBACK
		} else if item.ItemID == 17502584 { //trivia false
			if TriviaEventStarted {
				go c.AnswerTriviaQuestion(0)
			}
			goto FALLBACK
...
func (c *Character) CanUse(item *Item) bool {
	if item.MaxLevel < c.Level && item.MaxLevel != 0 {
		return false
	}
	if item.MinLevel > c.Level && item.MinLevel != 0 {
		return false
	}

	t := item.CharacterType

	if c.Type == 0x32 && (t == 0x32 || (t == 0x01 && item.BeastType == 2) || t == 0x03) { // MALE BEAST
		return true
	} else if c.Type == 0x33 && (t == 0x33 || (t == 0x02 && item.BeastType == 2) || t == 0x03) { // FEMALE BEAST
		return true
		...
		return true
	} else if c.Type == 0x3C && (t == 0x3C || (t == 0x01 && item.BeastType == 2) || t == 0x03 || t == 0x0A) { // DIVINE MALE BEAST
		return true
	} else if c.Type == 0x3D && (t == 0x3D || (t == 0x02 && item.BeastType == 2) || t == 0x03 || t == 0x0A) { // DIVINE FEMALE BEAST
		return true
		...
		return true
	} else if c.Type == 0x46 && (t == 0x46 || (t == 0x01 && item.BeastType == 2) || t == 0x03 || t == 0x0A) { // DARK LORD MALE BEAST
		return true
	} else if c.Type == 0x47 && (t == 0x47 || (t == 0x02 && item.BeastType == 2) || t == 0x03 || t == 0x0A) { // DARK LORD FEMALE BEAST
		return true
		...
		return true
	} else if t == 0x00 || t == 0x14 { //All character Type
		return true
	}
...
func (c *Character) CastSkill(attackCounter, skillID, targetID int, cX, cY, cZ float64) ([]byte, error) {
	...
	/*coordinate := ConvertPointToLocation(c.Coordinate)
	trg := GetFromRegister(user.ConnectedServer, c.Map, uint16(targetID))
	/*if trg != nil {

		targetai := GetFromRegister(user.ConnectedServer, c.Map, uint16(targetID)).(*AI)
		coordinateai := ConvertPointToLocation(targetai.Coordinate)
		distance := utils.CalculateDistance(coordinate, coordinateai)
		if distance > float64(skillInfo.RangeDistance)+3 {
			canCast = false
		}
	}*/
...
func (c *Character) DealInfection(ai *AI, character *Character, skillID int) {
	...
	if ai != nil { //AI BUFF
		//c.ApplyBuffEffectToAi(ai, int(infection.ID), skillInfo.PoisonDamage+skillInfo.AdditionalPoisonDamage, int64(duration))
...
func (c *Character) DealDamage(ai *AI, dmg int, isSkill bool) {
	...
	if ai.HP <= 0 { // ai died
		...
		//----------------------------------------------
		//-----------------Season Dungeon---------------------
		if SeasonCaveMobsCounter[server] == nil && c.Map == 212 && c.Socket.User.UserType != 5 {
...
func (c *Character) PoisonDamage(damage int) {
	...
	r.Insert(utils.IntToBytes(uint64(c.PseudoID), 2, true), 5)   // character pseudo id
...
func (c *Character) ResetPlayerSkillBook() {
	...
	for j := 0; j < 5; j++ {
		if skillSlots.Slots[j].BookID != 0 {
			//log.Print(fmt.Sprintf("BOOKID: %d"), skillSlots.Slots[j].BookID)
	...
	for k := 0; k < len(BookIDs); k++ {
		//log.Print(fmt.Sprintf("BOOKID: %d"), BookIDs[k])
		skillInfos := SkillBooks[BookIDs[k]].SkillTree
		set := &SkillSet{BookID: BookIDs[k]}
		c := 0
		for i := 1; i <= 24; i++ { // there should be 24 skills with empty ones
			//log.Print(fmt.Sprintf("SkillLen: %d"), len(skillInfos))
			if len(skillInfos) <= c {
				set.Skills = append(set.Skills, &SkillTuple{SkillID: 0, Plus: 0})
			} else if si := skillInfos[c]; si.Slot == i {
...
func (c *Character) LoadReturnQuests(questID, state int) ([]byte, error) {
	...
	if err != nil {
		log.Print(fmt.Sprintf("LoadError: %s", err.Error()))
	}
	if quest == nil {
		return nil, nil
	}
...
func (c *Character) GetQuestNPCID(NPCID int64) (int, error) {
	...
	filtered := make([]*NpcPosition, 0)
	for _, pos := range GetNPCPostions() {
		if c.Map == pos.MapID && pos.IsNPC && !pos.Attackable && pos.NPCID == int(NPCID) {
			filtered = append(filtered, pos)
		}
	}
	if len(filtered) == 0 {
...
func (c *Character) ShowEventsDetails() {
	...
	amount := float64((EXP_RATE - 1))
	EVENT_DETAILS.Overwrite(utils.FloatToBytes(amount, 4, true), 6)
	time := uint64(EXP_RATE_TIME) //time-seconds
	EVENT_DETAILS.Overwrite(utils.IntToBytes(time, 4, true), 10)
	c.Socket.Write(EVENT_DETAILS)
	EVENT_DETAILS = utils.Packet{0xAA, 0x55, 0x0A, 0x00, 0xE7, 0x01, 0x00, 0x00, 0x90, 0x3F, 0x30, 0x00, 0x00, 0x00, 0x55, 0xAA}
	amount = float64((DROP_RATE - 1))
	EVENT_DETAILS.Overwrite(utils.FloatToBytes(amount, 4, true), 6)
	time = uint64(DROP_RATE_TIME) //time-seconds
	EVENT_DETAILS.Overwrite(utils.IntToBytes(time, 4, true), 10)
	c.Socket.Write(EVENT_DETAILS)
	EVENT_DETAILS = utils.Packet{0xAA, 0x55, 0x0A, 0x00, 0xE7, 0x02, 0x00, 0x00, 0x90, 0x3F, 0x30, 0x00, 0x00, 0x00, 0x55, 0xAA}
	amount = float64((GOLD_RATE - 1))
	EVENT_DETAILS.Overwrite(utils.FloatToBytes(amount, 4, true), 6)
