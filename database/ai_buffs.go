...
func FindBuffByAIID(buffID, aiID int) (*AiBuff, error) {
	...
	if err := pgsql_DbMap.SelectOne(&buff, query, buffID, aiID); err != nil {
...
func FindBuffsByAiPseudoID(aiID uint16) ([]*AiBuff, error) {
	...
	if _, err := pgsql_DbMap.Select(&buffs, query, aiID); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func DeleteBuffsByAiPseudoID(aiID uint16) ([]*AiBuff, error) {
	...
	_, err := pgsql_DbMap.Exec(query, aiID)
	if err != nil {
		return nil, err
	}
...
func DeleteBuffByAiPseudoID(aiID uint16, buffID int) ([]*AiBuff, error) {
	...
	_, err := pgsql_DbMap.Exec(query, buffID, aiID)
	if err != nil {
		return nil, err
	}
