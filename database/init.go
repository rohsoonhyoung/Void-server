...
func InitPostgreSQL() error {
	...
	pgsql_DbMap.AddTableWithNameAndSchema(Quest{}, "hops", "characters_quests").SetKeys(false, "id", "character_id")
	...
	pgsql_DbMap.AddTableWithNameAndSchema(BannedRegion{}, "hops", "banned_regions").SetKeys(true, "id")
	...
	pgsql_DbMap.AddTableWithNameAndSchema(QuestionsItem{}, "data", "aso_trivia").SetKeys(false, "id")
	...
	pgsql_DbMap.AddTableWithNameAndSchema(User{}, "hops", "account").SetKeys(true, "id")
...
func resetDB() error {
	...
	query = `update hops.account set ip = $1, server = 0`
	if _, err := pgsql_DbMap.Exec(query, ""); err != nil {
		if err == sql.ErrNoRows {
			return nil
		}
...
func getAll() error {
	...
	callBacks := []func() error{getScripts, getHaxCodes, getEnhancements, GetBuffInfections, GetExps, GetAllCheckinRewards, GetBabyPets,
		GetRelics, getQuestionsItem, getAllQuests, getFiveAreas, getCookingItems,
