...
var (
	...
	GUILD_MEMBER_INFO = utils.Packet{0xAA, 0x55, 0x00, 0x00, 0x83, 0x08, 0x00, 0x00, 0x10, 0x27, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x49,
		0x2A, 0x00, 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0A, 0x32, 0x30, 0x31, 0x38, 0x2D,
...
func (g *Guild) GetMemberInfo(member *Character) []byte {
	...
	data.Insert(utils.IntToBytes(uint64(member.ClanGoldDonation), 4, true), index2) //gold donations
	index2 += 4
...
func (g *Guild) GetData(issuer *Character) ([]byte, error) {
	...
	if g.MemberCount > MAX_GUILD_MEMBERS && issuer.Socket != nil && issuer.IsOnline {
		issuer.Socket.Write(messaging.SystemMessage(messaging.MAX_GUILD_MEMBERS_REACHED))
	}
...
func FindGuildByName(name string) (*Guild, error) {
	...
	if err := pgsql_DbMap.SelectOne(&g, query, name); err != nil {
...
func FindGuildByID(id int) (*Guild, error) {
	...
	if err := pgsql_DbMap.SelectOne(&g, query, id); err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
func GetAllGuilds() error {

	var g []*Guild
	query := `select * from hops.guilds`

	if _, err := pgsql_DbMap.Select(&g, query); err != nil {
		if err == sql.ErrNoRows {
			return nil
		}
		return fmt.Errorf("GetAllGuilds: %s", err.Error())
	}
	for _, guild := range g {
