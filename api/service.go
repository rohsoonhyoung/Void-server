...
func (s *ApiService) Register(ctx context.Context, req *RegisterRequest) (*RegisterResponse, error) {
	...
	user = &database.User{
		BankGold:  0,
		CreatedAt: null.NewTime(time.Now(), true).Time.String(),
		Mail:      req.Mail,
		NCash:     0,
...
func (s *ApiService) GetTavern(ctx context.Context, req *Empty) (*GetTavernResponse, error) {
	...
	items := funk.Values(database.TavernItems).([]*database.TavernItem)
	items = funk.Filter(items, func(i *database.TavernItem) bool {
		return i.IsActive
	}).([]*database.TavernItem)
	...
	for _, i := range items {
		title := titles[i.ID/1000]
		info, ok := database.GetItemInfo(int64(i.ItemID))
		if !ok {
			continue
		}
		...
		}{info.Name, i.Price, quantity, i.IsNew, i.IsPopular}
